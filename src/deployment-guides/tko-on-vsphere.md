...
* A datastore with at least:
  * 2GB to store the Tanzu Kubernetes Grid OVAs
  * 60GB to store the virtual disks for the management control plane
...
instructions, then reboot and log in again.
...
> ✅  Curious about how to retrieve the values file for `contour` and other
> kApps? Go [here](#extra-retrieving-kapp-values-files) to learn more.
...
<!-- markdownlint-enable MD037 -->
  ...
  * Delete the auto generated password and replace it with the user
    provided value:
...
5. Install the Harbor Package using below command:

    ```sh
    tanzu package install harbor --package-name harbor.tanzu.vmware.com \
      --version <AVAILABLE-PACKAGE-VERSION> \
      --values-file < path to harbor-data-values.yaml> \
      --namespace tanzu-system-registry \
      --create-namespace
    ```

6. To address a known issue, patch the Harbor package by following the steps in the Knowledge Base article The [harbor-notary-signer pod fails to start](https://kb.vmware.com/s/article/85725)

7. Confirm that the harbor package has been installed, the status must change to “Reconcile succeeded”
...
##### Prerequisites for installing TMC
...
##### Prerequisites for installing TO
...
### EXTRA: Retrieving kApp Values Files
