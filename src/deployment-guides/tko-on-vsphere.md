...
* Depending on the OS flavor of the bootstrap VM, [download](https://customerconnect.vmware.com/en/downloads/details?downloadGroup=TKG-151&productId=988&rPId=49705)and configure the following packages. As part of this documentation, refer to the section to configure required packages on Photon Machine.
...
* [Download](https://customerconnect.vmware.com/en/downloads/details?downloadGroup=TKG-151&productId=988&rPId=49705) the following OVA and import to vCenter. Convert the imported VMs to templates.
...
## Tanzu Kubernetes Operations: Deployment Overview
...
1. Login to **vCenter** > **Home** > **Content** **Libraries** 
...
1. On the Ready to complete page, review the page and click **Finish**
    ...
    ![](img/tko-on-vsphere/6-AVI-ova-config.png)
...
1. Administrator account setup: Set admin password and click on **Create Account**.  
    ![](img/tko-on-vsphere/7.ALB-login.png)  

1. On the Welcome page,

1. Under System Settings: Set **backup Passphrase** and provide **DNS** information and click Next  
    ![](img/tko-on-vsphere/8.ALB-Welcome-screen.png)
1. Under Email/SMTP: Provide **Email** or **SMTP** information  
    ![](img/tko-on-vsphere/9.ALB-Welcome-email-smtp.png)
1. Under Multi-Tenant: Configure settings as shown below and click on Save  
    - IP Route Domain: Share IP route domain across tenants  
    - Service Engines are managed within the: Provider (Shared across tenants)  
    - Tenant Access to Service Engine: Read  
    ![](img/tko-on-vsphere/10.ALB-Welcome-Multi-tenant.png)
...
![](img/tko-on-vsphere/11.ALB-NTP.png)
...
![](img/tko-on-vsphere/12.ALB-Licensing.png)
...
1. Log in to the primary NSX ALB controller > Navigate to **Administrator** > **Controller** > **Nodes,** and click **Edit**

    ![](img/tko-on-vsphere/13.ALB-Nodes.png)

1. Specify the **Name** and set the **Controller Cluster IP** and click on **Save**. This IP address should be from the NSX ALB management network. 
  ![](img/tko-on-vsphere/14.ALB-controller-vip.png)

1. Now deploy 2nd and 3rd NSX ALB Node, using steps provided [Deploy NSX Advanced load balancer](#deploynsxalb)

1. Log into the Primary NSX ALB controller using the Controller Cluster IP/FQDN, navigate to **Administrator** > **Controller** >  **Nodes,** and click **Edit**. The Edit Controller Configuration popup appears

1. In the Cluster Nodes field, enter the IP address for the 2nd and 3rd controller and click on **Save**

    ![](img/tko-on-vsphere/15.ALB-additional-nodes.png)
    ...
    ![](img/tko-on-vsphere/16.ALB-Controller-initialization.png)
...
![](img/tko-on-vsphere/17.ALB-cluster.png)
...
1. Login to NSX ALB Controller > **Templates** > **Security** > **SSL/TLS Certificates**

1. Click on **Create** and Select **Controller Certificate**

1. You can either generate a Self-Signed certificate, generate CSR or import a certificate. For the purpose of this document, a self-signed certificate will be generated.

1. Provide all required details as per your infrastructure requirements, and under the Subject Alternate Name (SAN) section, provide IP and FQDN of all NSX ALB controllers including NSX ALB cluster IP and FQDN, and click on Save

    ![](img/tko-on-vsphere/18.ALB-Certificate.png)

1. Once the certificate is created, capture the certificate contents as this is required while deploying the TKG management cluster. 
  To capture the certificate content, click on the “Download” icon next to the certificate, and then click on “Copy to clipboard” under the certificate section
    ...
    ![](img/tko-on-vsphere/19.ALB-Certificate-contents.png)

1. To replace the certificate navigate to **Administration** > **Settings** > **Access** **Settings**, and click the pencil icon at the top right to **edit** the System Access Settings, replace the SSL/TSL certificate and click on **Save**

    ![](img/tko-on-vsphere/20.ALB-Cert-replace.png)
...
1.  Login to NSX ALB > Infrastructure > Clouds > Create > VMware vCenter/vSphere ESX  
    ![](img/tko-on-vsphere/21.ALB-Clouds.png)

2.  Provide Cloud Name and click on Next  
    ![](img/tko-on-vsphere/22.ALB-Clouds-1.png)
...
3.  Under the Infrastructure pane, provide vCenter Address, username, and password and set AccessPermission to "Write" and click on Next  
    ![](img/tko-on-vsphere/23.ALB-Clouds-2.png)

4.  Under the Datacenter pane, Choose the Datacenter for NSX ALB to discover Infrastructure resources  
    ![](img/tko-on-vsphere/24.ALB-Clouds-3.png)

5.  Under the Network pane, choose the NSX ALB ManagementNetwork for Service Engines and provide a StaticIPpool for SEs and VIP and click on Complete![](img/tko-on-vsphere/25.ALB-Clouds-4.png)

6.  Wait for the status of the Cloud to configure and status to turn Green  
    ![](img/tko-on-vsphere/26.ALB-Clouds-5.png)
...
8.  Provide a name for the TKG management Service Engine group and set below parameters  
    ...
    ![](img/tko-on-vsphere/27.ALB-SE.png)
    ...
    ![](img/tko-on-vsphere/28.ALB-SE-Group2.png)  
...
9.  Follow steps 7 and 8 to create another Service Engine group for TKG workload clusters. Once complete there must be two service engine groups created  
    ![](img/tko-on-vsphere/29.ALB-SE-Group3.png)
...
1. Login to NSX ALB > **Infrastructure** > **Networks**
1. Select the appropriate Cloud
1. All the networks available in vCenter will be listed

    ![](img/tko-on-vsphere/30.ALB-Networks-1.png)

1. Click on the edit icon next for the network and configure as below. Change the details provided below as per tour SDDC configuration 
    ...
    **Note:** Not all networks will be auto-discovered and for those networks, manually add the subnet.
    ...
    ![](img/tko-on-vsphere/31.ALB-Networks-2.png)  

1. Once the networks configured, the configuration must look like below  
    ![](img/tko-on-vsphere/32.ALB-Networks-3.png)
1. Once the networks are configured, set the default routes for all VIP/Data networks  
    ...
    ![](img/tko-on-vsphere/33.ALB-Networks-4.png)  


#### Create IPAM & DNS Profile in NSX ALB and attach it to Cloud
...
* Login to NSX ALB > Templates> IPAM/DNS Profiles> Create > IPAM Profile and provide below details and click on Save  
    ...
    ![](img/tko-on-vsphere/34.ALB-IPAM.png)

* Click on the **Create** > **DNS Profile** and provide the **Domain Name**

    ![](img/tko-on-vsphere/34.ALB-DNS.png)
...
* Attach the IPAM & DNS profiles to the “tanzu-vcenter-01” cloud. Navigate to Infrastructure > Clouds > Edit the tanzu-vcenter-01 cloud > Under IPAM/DNS section  choose the IPAM and DNS profiles that we created above and Save the configuration  
    ![](img/tko-on-vsphere/35.ALB-IPAM-DNS.png)  
...
1. Access Tanzu UI wizard by opening a browser and entering  http://<bootstrapper-ip:port/  
    ![](img/tko-on-vsphere/36-Mgmt-cluster-1.png)
1. Click Deploy on the VMware vSphere tile
1. On the "IaaS Provider" section, enter the IP/FQDN and credentials of the vCenter server where the TKG management cluster will be deployed  
    ![](img/tko-on-vsphere/37-Mgmt-cluster-2.png)
1. Click on connect and accept the vCenter Server SSL thumbprint
1. If you are running on a vCenter 7.x environment, you would get below popup, select “DEPLOY TKG MANAGEMENT CLUSTER” to proceed further  
    ![](img/tko-on-vsphere/38-Mgmt-cluster-3.png)
...
1. Click Next  
    ...
    ![](img/tko-on-vsphere/39-Mgmt-cluster-4.png)  
...
1. On the Management cluster settings section provide the following details,
    ...
    * Click Next
    ...
    ![](img/tko-on-vsphere/40-Mgmt-cluster-5.png)

1. On the NSX Advanced Load Balancer section, provide the following:
    ...
    * Click Next.
    ...
    ![](img/tko-on-vsphere/41-Mgmt-cluster-6.png)
...
                
1. On the Metadata page, you can specify location and labels and click Next, this is optional  
    ![](img/tko-on-vsphere/42-Mgmt-cluster-7.png)
1. On the Resources section, specify the resources to be consumed by TKG management cluster and click on Next  
    ![](img/tko-on-vsphere/43-Mgmt-cluster-8.png)

1. On the Kubernetes Network section, select the TKG Management Network (`tkg_mgmt_pg`) where the control plane and worker nodes will be placed during management cluster deployment. Ensure that the network has DHCPservice enabled.
    ...
    * For vSphere, you must manually add the CIDR of TKG Management Network and Cluster VIP networks which includes the IP address of your control plane endpoints, to `TKG_NO_PROXY`.
    ...
    ![](img/tko-on-vsphere/44-Mgmt-cluster-9.png)

1. Optionally Specify Identity Management with OIDC or LDAP -   For the purpose of this document, Identity management integration has been disabled.
    ...
    ![](img/tko-on-vsphere/45-Mgmt-cluster-10.png)  
...
1. Select the OS image that will be used for the management cluster deployment.  
    ...
    ![](img/tko-on-vsphere/46-Mgmt-cluster-11.png)


1. Check the “Participate in the Customer Experience Improvement Program”, if you so desire and click Review Configuration
1. Review all the configuration, once reviewed, you can either copy the command provided and execute it in CLI or proceed with UI to Deploy Management Cluster.  
    When the deployment is triggered from the UI, the installer wizard displays the deployment logs on the screen.  
    ![](img/tko-on-vsphere/47-Mgmt-cluster-12.png)
...
* Service engine status in NSX ALB: Below snippet shows the service engines status. They are in initializing status for sometime and the changes to Up  
    ![](img/tko-on-vsphere/48-Mgmt-cluster-11.png)
* Service Engine Group Status in NSX ALB: As per our configuration, we can see that the virtual service required for TKG clusters control plane HA will be hosted on service engine group `tkg-mgmt-segroup-01`.  
    ![](img/tko-on-vsphere/50-Mgmt-cluster-15.png)
* VIrtual Service status in NSX ALB: We have configured the cluster with Production type which deployed 3 Control plane nodes, which are placed behind the cluster VIP here.
    ![](img/tko-on-vsphere/51-Mgmt-cluster-16.png)

    ![](img/tko-on-vsphere/52-Mgmt-cluster-17.png)

* Once the TKG management cluster is successfully deployed, you will find this in the Tanzu Bootstrap UI  
    ![](img/tko-on-vsphere/53-Mgmt-cluster-18.png)
...
* The installer will automatically set the context to the TKG Management Cluster on the bootstrap machine. Now you can access the TKG management cluster from the bootstrap machine and perform additional tasks such as verifying the management cluster health and deploy the workload clusters, etc.
    ...
    ![](img/tko-on-vsphere/54-Mgmt-cluster-19.png)

* Use kubectl to get the status of the TKG Management cluster nodes  
    ![](img/tko-on-vsphere/55-Mgmt-cluster-20.png)
...
1. Navigate to the Clusters tab and click on the Create Cluster button.
    ...
    ![](img/tko-on-vsphere/65-shared-service-1.png)
...
2. Select the provisioner for creating the workload cluster(shared services cluster). Provisioner reflects the vSphere namespaces that you have created and associated with the Management cluster.
    ...
    ![](img/tko-on-vsphere/66-shared-service-2.png)
...
3. Enter a name for the cluster. Cluster names must be unique within an organization.
    ...
    ![](img/tko-on-vsphere/67-shared-service-3.png)
...
6. Select the High Availability mode for the control plane nodes of the workload cluster. For a production deployment, it is recommended to deploy a highly available workload cluster. 
    ...
    ![](img/tko-on-vsphere/70-shared-service-6.png)
...
7.  Customize the default node pool for your workload cluster.
    ...
    ![](img/tko-on-vsphere/71-shared-service-7.png)

8. Once the cluster is created, you can check the status from TMC. 

    ![](img/tko-on-vsphere/72-shared-service-8.png)
    ...
    ![](img/tko-on-vsphere/73-shared-service-9.png)
...
1.  Navigate to the Clusters tab and click on the Create Cluster button.
    ...
    ![](img/tko-on-vsphere/74-workload-cluster-1.png)
...
2. Select the provisioner for creating the workload cluster. Provisioner reflects the vSphere namespaces that you have created and associated with the Management cluster.
    ...
    ![](img/tko-on-vsphere/75-workload-cluster-2.png)

3. Enter a name for the cluster. Cluster names must be unique within an organization.

    Select the cluster group to which you want to attach your cluster. You can optionally enter a description and apply labels.

    ![](img/tko-on-vsphere/76-workload-cluster-3.png)
...
4.  On the configure page, specify the following:
    ...
    ![](img/tko-on-vsphere/77-workload-cluster-4.png)

5. Select the resources for backing this cluster. Provide the Resource Pool, VM folder and Datastore information. 

    ![](img/tko-on-vsphere/78-workload-cluster-5.png)

6. Select the High Availability mode for the control plane nodes of the workload cluster. For a production deployment, it is recommended to deploy a highly available workload cluster. 

    ![](img/tko-on-vsphere/79-workload-cluster-6.png)
...
7. Customize the default node pool for your workload cluster.
    ...
    ![](img/tko-on-vsphere/80-workload-cluster-7.png)

8. You can monitor the workload cluster creation from the TMC console. 
    ![](img/tko-on-vsphere/81-workload-cluster-8.png)
...
9. Cluster creation roughly takes 15-20 minutes to complete. After the cluster deployment completes, ensure that Agent and extensions health shows green.
    ...
    ![](img/tko-on-vsphere/82-workload-cluster-9.png)
...
![](img/tko-on-vsphere/83-ako-workload-1.png)
...
![](img/tko-on-vsphere/84-ako-workload-2.png)
...
You can now [configure SaaS services](./tko-saas-services.md/#configure-tanzu-saas-components-for-tanzu-for-kubernetes-operations) for the cluster and/or deploy user managed packages on this cluster.
