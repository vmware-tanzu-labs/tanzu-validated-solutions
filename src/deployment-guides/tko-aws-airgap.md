...
The following describes the steps to create your AWS environment and configure your network. Follow the steps in the order provided.
  ...
  ### Create an internet-restricted Virtual Private Cloud (VPC)
   Follow these steps to create an internet-restricted (offline) AWS VPC or refer to the [AWS VPC documentation](https://docs.aws.amazon.com/vpc/latest/userguide/working-with-vpcs.html). This offline VPC should not have a NAT or internet gateway. Create your offline VPC with private subnets only.
Steps to follow:

 **Step 01. Create a VPC**

* Log in to your AWS Console and select VPC service.
...
* Click **Action** and Edit DNS hostname
* Select DNS Hostname and click **Save**.

**Note:** If you are creating multiple offline VPCs, follow these instructions to create an [aws transit gateway](https://docs.aws.amazon.com/vpc/latest/tgw/tgw-getting-started.html).

### Add VPC endpoints into offline VPC

After you create the offline VPC, you must add the following endpoints to it. (VPC endpoints enable private connections between your VPC and supported AWS services.)
...
See [aws create endpoints service documentation](https://docs.aws.amazon.com/vpc/latest/privatelink/create-endpoint-service.html) for further details.
...
<_I added quotes around *true* - correct?_>
Using an internal load balancer scheme prevents the Kubernetes API server for the cluster from being accessed and routed over the internet. <_I reworded this note because the original seemed redundant. Is the meaning still correct?_>
