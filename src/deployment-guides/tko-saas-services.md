...
**Note:** If the Tanzu Kubernetes Grid clusters are behind a proxy, import the proxy configuration to Tanzu Mission Control and then attach the cluster using Tanzu Mission Control.
...
4. (Optional) If you are using a proxy to connect to the internet, you can configure the proxy settings by toggling the Set proxy option to yes, otherwise leave it disabled.
   ...
   For information on registering a cluster that is behind a proxy with Tanzu Mission Control, please see the [Appendix A](#appendix-a)
...
After you successfully register a management cluster, you can add any existing Tanzu Kubernetes Grid clusters that are currently managed by the management cluster to Tanzu Mission Control or you can deploy new workload clusters directly from Tanzu Mission Control.
...
10. Click **Create**. The proxy configuration is added to Tanzu Mission Control. Use this proxy configuration when you add a cluster that is sitting behind the proxy. Based on your environment, you can add multiple proxies in Tanzu Mission Control.  
    ![TMC Proxy List](./img/tko-saas-services/tmc-proxy-configuration03.png)
