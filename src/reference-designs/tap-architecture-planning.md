...
This architecture should give you an path to creating a production deployment of Tanzu Application Platform, however you should not feel constrained exactly this path if your specific use cases lead you to architectural differences.
...
|TAP-004  | Utilize a UI Cluster  | Utilizing a single build cluster with multiple run clusters creates the correct production for the build system vs separating into dev/test/qa/prod build systems. Additionally, it raises confidence that the container image does not change between environments.  It also enhances manageability versus having separate components. |  None

### Build Cluster Requirements
The Build Cluster is responsible for taking developer commits and applying a supply chain that will produce a container image and kubernetes resources for deploying on a run cluster.
...
* Total of minimum 16GB memory allotable available across cluster, at least 8gb per node
...
The run cluster will read the container image and kubernetes resources created by the build cluster and run them as defined in the Delivery object for each application.
...
In some cases a run cluster might not have some components that are not necessary for specific workloads. For example, if a run cluster is running applications that are not using Cloud Native Runtime, those components could be omitted.
Kubernetes requirements:
* LoadBalancer for ingress controller (1 external IP)
* Default storage class
* Total of minimum 16GB memory allotable available across cluster, at least 8gb per node
...
The UI cluster is designed to run the web applications for TAP. Specifically learning center, TAP GUI, and API Portal.
The run clusters requirements will be mostly driven by the respective applications that it will be running.  Horizontal and vertical scale will be determined based on the type of applications being scheduled. 
Kubernetes requirements:
...
excluded_packages:
 ...
 ```
### Personal Cluster Requirements
The personal cluster is for "inner loop" development iteration where developers are connecting via their IDE to rapidly iterate on new software features. The personal cluster operates distinctly from the outer loop infrastructure. Each developer should be given their own namespace within the personal cluster during their platform onboarding. 

![](img/tap-architecture-planning/personal-cluster.png)
...
To install a personal cluster, use the following package definition:
```yaml
profile: full 
excluded_packages:
 - accelerator.apps.tanzu.vmware.com
...
### In Cluster
Services can be deployed directly into the same cluster running Tanzu Application Service. This kind of deployment is more suited to personal environments. Two possible implementations include, 1) same namespace, 2) different namespaces. The diagram depicts the latter.
...
* Number of pods waiting to be scheduled
* Maximum number of seconds any pod has waited for scheduling
* Remaining allotable memory and CPU
UI components:
* Response time
* Availability
| Decision ID   | Design Decision   | Justification | Implication
