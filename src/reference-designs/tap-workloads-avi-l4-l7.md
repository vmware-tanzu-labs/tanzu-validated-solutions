...
VMware Tanzu Application Platform, often referred to as TAP, is an advanced application development platform equipped with a comprehensive suite of developer tools. It provides developers with a well-defined route to production, facilitating the swift and secure development and deployment of software. This platform is adaptable to any compliant public cloud or on-premises Kubernetes cluster, ensuring flexibility and ease of use in diverse computing environments.

In this blog, we will explore how we can leverage cartographer to template custom K8s  resources, that is creating new workload types, by modifying the [ClusterConfigTemplate](https://cartographer.sh/docs/v0.3.0/reference/template/#clusterconfigtemplate) for deploying a workload which can be exposed outside the cluster using either AVI L4 or L7. 
...
- Install the `yq` cli on client machine. `yq` is a lightweight and portable command-line YAML processor. You can download yq by clicking [here](https://github.com/mikefarah/yq/releases/) and run below commands to install.
    ```bash
...
1. Extract the `.spec.ytt` field from this file and create another file by running:
    ...
    ```
1. In the next step, you will add the `Ingress` resource snippet to `spec-ytt.yaml`. This step provides a sample `Ingress` resource snippet below. Make the following edits before adding the `Ingress` resource snippet to `spec-ytt.yaml`:
    ...
    - `ingressClassName: #@ data.values.params.ingressClass` : This parameter is passed from the `workload.yaml` params section which specifies the ingress class name, to be used while creating the ingress object.
    - if `data.values.params.serviceType != "LoadBalancer"`, This condition is added to create the ingress resource based on the value assigned to the `serviceType` parameter from the `workload.yaml`. 
...
1. Add the above to the `.spec.ytt` property in `avi-l4-l7-server-template.yaml` by running:
    ```bash
    SPEC_YTT=$(cat spec-ytt.yaml) yq eval -i '.spec.ytt |= strenv(SPEC_YTT)' avi-l4-l7-server-template.yaml
    ```

1. Change the name of the `ClusterConfigTemplate` to `avi-l4-l7-server-template` by running:
    ```bash
...
1. Verify the new `ClusterConfigTemplate` is in the cluster by running:
    ...
    worker-template              82m
    ```
1. Add the new workload type to the `tap-values.yaml`. The new workload type is named `avi-l4-l7-server` and the `cluster_config_template_name` is `avi-l4-l7-server-template`.
    ```bash
...
Now we deploy a server workload using AVI L7 and expose it externally. Below diagram shows the flow of external traffic entering the cluster through AVI L7 ingress and passing through the successive K8s objects before reaching the application.

![Traffic flow of Server workload deployed behind AVI L7](img/tap-workloads-avi-l4-l7/tap-workload-avi-l7.png)
...
Now, lets deploy a server workload using AVI L4 and expose it externally. The below diagram shows the flow of external traffic entering the cluster through AVI L4 IP and passing through the successive K8s objects before reaching the application.

![Traffic flow of Server workload deployed behind AVI L7](img/tap-workloads-avi-l4-l7/tap-workload-avi-l4.png)
...
1. Apply the below workload YAML to deploy the server application. The value assigned serviceType parameter is `LoadBalancer`, this creates a service of type LoadBalancer and the ingress object creation is skipped.
    ...
    source:
        ...
        url: <Git url with Application code>
    ```
1. After the process finishes, you will see the Deployment and LoadBalancer Service(L4) resources similar to:
    ```bash
    # kubectl get ingress,svc,deploy -l carto.run/workload-name=tanzu-java-web-app
    ...
    deployment.apps/tanzu-java-web-app-00001-deployment   1/1     1            1           3d1h
    ```
1. Use the IP address from the above service object output to the access the application, as shown below:
    ![app-l4-accessibility](img/tap-workloads-avi-l4-l7/app-l4.png)

## Conclusion
We have uncovered a powerful approach to streamline Kubernetes operations using supply chain objects. The supply chain does the heavy lifting to generate the necessary K8s Ingress objects. Furthermore, during app deployment this feature helps Platform operators to generate a spectrum of K8s objects(configmaps, pvc’s, secrets..etc) by defining the required conditions within the supply chain.