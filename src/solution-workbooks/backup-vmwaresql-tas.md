# Backup & Restore VMware SQL with Mysql instances for Tanzu Application Service

VMware SQL with MySQL for TAS enables developers to provision and use dedicated instances MySQL database on demand. When you install VMware SQL with MySQL for TAS, the tile deploys and maintains a single service broker that integrates VMware SQL with MySQL for TAS with Ops Manager.

In this blog we discuss about how to backup a running Mysql service instance into the desired s3 (AWS, Ceph, GCS, Azure, Minio) compatible storage and restore it back to a new service instance. 

- In VMware SQL with MySQL for TAS `v2.9.0` to `v2.9.2` and above, the `ApplicationDeveloperBackupRestore(adbr)` CF plugin helps to trigger both backup and recovery processes while storing the backup artifacts organized under subfolders in external storage. They are now stored under `p.mysql > service-instance_GUID > yyyy > mm > dd` path

- The approach helps to automate some of the steps through CF cli as opposed to the manual process for taking backup/restore of service instances on TAS for MYSQL VMs using mysqldump.
- The approach helps to take logical backups and not any physical backup.  

![VMware SQL with MySQL for TAS Backup](img/backup-vmwaresql-tas/mysql-tas-backup.png)

## Pre-requisites

- Install `ApplicationDataBackupRestore` plug-in for the Cloud Foundry Command Line Interface (cf CLI) tool. 
    - Install the `adbr` plugin by running the below command:
        ```bash
        ...
        ```
    - Validate that `adbr` plugin is listed under cf plugin list. 
        ```bash
        cf plugins
...
- Configure a S3 compatible storage to store Service instance data backup. For more information on supported blob store for MySQL backups, see `configuring backups` section in [Installing and configuring VMware SQL with MySQL for Tanzu Application Service](https://docs.vmware.com/en/VMware-SQL-with-MySQL-for-Tanzu-Application-Service/3.1/mysql-for-tas/install-config.html#configuring-backups-12).

> **Note**: If you use `Minio` as s3 storage for backups, then you need to enable `Force path style access to bucket` option under the `Backups` configuration in the `VMware SQL with MySQL for Tanzu Application Service` tile, particularly when accessing MinIO over an ip address. For more information, see `Back Up to Amazon S3 or Ceph` section in the [Configuring automated backups](https://docs.vmware.com/en/VMware-SQL-with-MySQL-for-Tanzu-Application-Service/3.1/mysql-for-tas/automated-backup.html#configure-backups-in-tanzu-operations-manager-6).

## Backup a Service Instance created with MySQL for TAS

In this step, we will backup the data on a service instance created with MySQL for TAS Service Instance Tile. For this demonstration purpose, we used a Spring Petclinic application on TAS for MySQL VM v3.1.1-build.21. 
...
1. Retrieve the service instance details that is planned to be back-up:
    ```bash
    ...
    ```
1. Back up the service instance using `adbr` plugin by running the below command:
    ```bash
...
1. Validate the status of the Service instance backup from the CF CLI
    ...
    ```
1. Validate that backup file is available in the blob store, minio in this use case:

    ![Backup data in minio](img/backup-vmwaresql-tas/backup-data-minio.png)

## Restore the backup to a new Service Instance

In this step, we will restore the backup to a new service instance with an empty database. Backup fails if any of the following steps are executed:
- Creating service key for the instance
- Configuring service instance for Multi-Site replication
- Generating any automatic backup on the existing Service Instance

Follow the below steps to create a new Service Instance and restore the backup from previous instance.
...
1.  Verify if the service instance has been created successfully
    ...
    There is no upgrade available for this service.
    ```
1. Check the available backup artifacts for your service instance by running the below command record the backup id from the output.
    ```bash
    ...
    ```
1. Restore the backup to the newly-created service instance
    ```bash
    ...
    OK
    ```
1. Check status of the restore process and confirm that it is successfully restored
    ```bash
    ...
    ```
1. Unbind the application from the source Service Instance and re-bind to the new target Service Instance where the backup was restored.
    ```bash
    ...
    ```
1. Confirm if the `VCAP_SERVICES` populate the backup DB information correctly
    ```bash
   # cf env spring-petclinic
    ...
    ```
1. Access the application and confirm if we are able to get the old data backed up from the source instance
    ```bash
    ...
    ```
1. Access the application from browser and confirm its showing the previous data.
